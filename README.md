...
| **서류를 모국어로 풀어요** | 고지서 사진 한 장이나 PDF 파일이면 글자를 읽고, 안내 자료를 찾고, 원본과 **다시 대조**한 뒤 항목마다 출처를 붙여 설명합니다. |
...
| **서류 사진 설명** | 하단 `도움` 탭 → `서류 사진 찍어 물어보기` → `사진 찍기`. 데모용 가상 가스 청구서로 실제 분석이 돌아갑니다(약 15초). 가지고 있는 사진이나 PDF(2.5MB 이하)는 `파일 올려서 물어보기`로 올리세요. |
...
| 단계 | 하는 일 |
|---|---|
| ① 서류 읽기 | 사진·PDF에서 종류·금액·납기일·주요 글자를 읽습니다(OCR). PDF는 여러 쪽이어도 원본 그대로 모델에 넘깁니다. 이름·주소·고객번호는 뽑지 않습니다. |
...
2. **초안을 보지 않은 채** 원본 사진·PDF와 자료만으로 질문에 답합니다.
3. 두 답이 어긋난 항목이 있을 때만 수정합니다.

사진·PDF와 원문은 저장하지 않습니다. 분석하는 동안 메모리에만 두고, 끝나면 지웁니다.
