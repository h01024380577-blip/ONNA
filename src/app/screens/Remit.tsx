...
import { ThinkingCard } from '../AgentSteps'
  ...
  return (
    <>
      ...
      <div className="appFoot">
        ...
        <button className="btn" disabled={amount <= 0} onClick={() => dispatch({ type: 'CONFIRM_AMOUNT' })}>
...
export function B3() {
  const { state, dispatch, p, t, krw, local, day } = useApp()
  const fx = FX[p.currency]
  const [bio, setBio] = useState(false)
  const amount = state.draftAmount
  // ⑥ 타이밍 — 지문(EXECUTE)이 이 값대로 실행한다
    ...
    <>
      <div className="sheetWrap">
        <div className="sheet">
          ...
          <button className="btn" style={waitOnly ? { marginTop: 12 } : undefined} onClick={run}>
            <Icon name="finger" size={19} style={{ marginRight: 8 }} />{waitOnly ? t('tm.btnSchedule') : t('b3.btn')}
          </button>
        </div>
      </div>
      {bio && (
...
export function B5() {
  ...
  const rest = state.timing?.rule === 'split' ? laterParts(state.timing)[0] : undefined
      ...
      <div className="appBody">
        ...
        {rest && (
          <div className="note amber">
            ...
            <span>{t('b5.rest', { amount: krw(rest.amount), date: day(rest.when) })}</span>
          </div>
        )}
